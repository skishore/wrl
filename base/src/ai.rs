...
fn FindPath(ctx: &mut Ctx, target: Point, kind: PathKind) -> bool {
    ...
    let skip = match kind {
        ...
        PK::Enemy | PK::Hide | PK::Flee | PK::Rest | PK::Explore | PK::None => 0,
    };
...
fn UpdateFlightState(ctx: &mut Ctx) -> bool {
    ...
    let Some(threat) = bb.threats.hostile.first() else { return false };
...
fn FightAgainstThreats() -> impl Bhv {
    ...
    .on_tick(ClearTargets)
    .on_tick(|x| x.blackboard.threats.state = FightOrFlight::Fight)
...
fn EscapeFromThreats() -> impl Bhv {
    ...
    .on_tick(|x| x.blackboard.threats.state = FightOrFlight::Flight)
