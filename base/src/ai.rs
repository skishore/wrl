...
impl Blackboard {
    fn new(predator: bool, rng: &mut RNG) -> Self {
        Self {
            ...
            till_weary_: rng.gen_range(0..MAX_WEARY_),
            finding_food_: false,
            finding_water: false,
            getting_rest_: false,
            hunger: false,
...
//
//  5. In the flight subtree, while we're watching a target closely, since we
//     don't set up dirs in LookForThreats, we'll repeatedly do a "hide" scan
//     on each turn that we watch them.
//
//  6. Nit: when we're following a path to water, if we end next to two water
